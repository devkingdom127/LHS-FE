...
class FindStaff extends React.Component {
    ...
    handleSearch= async(e)=>{
        ...
        var skip = (this.state.curPage-1)*10;
        var Authorization = 'Bearer '.concat(window.localStorage.getItem('token'));
            if(this.state.searchField==="Name"){
                var res = await callApi("GET", "/v1/LHS/hospital/staffByName/?skip="+skip+"&name="+sendData+"&sort=availability&filter=Commitment&value=Full Time&sortValue=true", Authorization);
                ...
                if(sendData==="")
                    ...
                    var res = await callApi("GET", "/v1/LHS/hospital/staffByEducation/?skip="+skip+"&education="+sendData+"&sort=rating&filter=Commitment&value=Part Time&sortValue=-1", Authorization);
                    if(res.status===404){
                        this.setState({staffs:[], searchResultNone:true});
                    }
    ...
    handleSort=(event)=>{
        ...
        if(sorting === "availability" )
            this.setState({sort:event.target.value,sortValue:true});
        else if(sorting === "rating")
            this.setState({sort:event.target.value,sortValue:1});
    }
                ...
                {!this.state.searchResultNone?this.state.staffs.map((staff)=>{
                    return(<div className="staffOne row">
                        ...
                        <div className="col-md-2 col-sm-2 staffOneData">
                            <div className="review_info">
                                <div className="middle_line"></div>
                                <div className="reviewBTN" onClick={()=>this.openReview(staff)}>REVIEW</div>
                            </div>
                        </div>
                    </div>
                )})
                    ...
                    <div  className="w3-modal-content ssu2_modal2">
                        <div className="w3-container">
                            <div className="ssu2_modal1_text1">
                                <p className="reviewStaff">{this.state.selectStaff.name}</p>
                                <p className="reviewEach">{this.state.selectStaff.ratings.ratingSum} Review</p>
                                <p className="reviewMark reviewMarkhidden">{this.state.selectStaff.ratings.totalRatings}</p>
